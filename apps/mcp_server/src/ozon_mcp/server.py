...
from ozon_mcp.tools import register_tools

log = logging.getLogger("ozon.mcp")
...
def main() -> int:
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [ozon-mcp] %(levelname)s %(message)s",
        stream=sys.stderr,
        force=True,
    )
    host = mcp.settings.host
    port = mcp.settings.port
    log.info("starting ozon-helper MCP server on %s:%s", host, port)
