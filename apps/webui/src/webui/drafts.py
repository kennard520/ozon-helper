...
LOW_RES_VIDEO_RE = re.compile(r"/mp4/(?:144|240|360|480)p(?:/|$)", re.IGNORECASE)
...
def to_ozon_import_item(draft: dict[str, Any], *, validate: bool = True) -> dict[str, Any]:
    ...
    if video_url and not video_publish_skip_reason(video_url, sr):
...
def video_publish_skip_reason(video_url: object, source_raw: object | None = None) -> str:
    """Return a warning when a source video is known to be too small for Ozon."""
    urls = [str(video_url or "").strip()]
    if isinstance(source_raw, str):
        source_raw = loads_json(source_raw, {})
    if isinstance(source_raw, dict):
        urls.append(str(source_raw.get("video_url") or "").strip())
    urls = [u for u in urls if u]
    if not urls:
        return ""
    if any(LOW_RES_VIDEO_RE.search(u) for u in urls):
        return "视频分辨率低于 Ozon 要求（例如 WB 360p/480p，高度不足 640px），发布时已跳过视频；请上传或 AI 生成更高清的视频"
    return ""
