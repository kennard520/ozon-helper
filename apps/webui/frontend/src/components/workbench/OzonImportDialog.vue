    ...
    <div class="ozon-import-dialog">
      <div class="sku-row">
        <el-input
          ...
          placeholder="输入 Ozon 数字 SKU，例如 4998185789"
      ...
      <div v-if="state === 'loading'" class="state-message is-loading">
        {{ activeAction === 'sync' ? '正在同步当前店铺商品…' : '正在导入 Ozon 商品…' }}
      </div>

      <div v-else-if="state === 'error'" class="state-message is-error">
        {{ errorMessage }}
      </div>
      ...
      <template v-else-if="state === 'done'">
        <div class="state-message is-done">
          <template v-if="completedAction === 'sync'">
            同步完成：共拉取 {{ syncResult.pulled ?? 0 }} 个商品，新增 {{ syncResult.created ?? 0 }} 个，更新 {{ syncResult.updated ?? 0 }} 个。
          </template>
          <template v-else>
            {{ importResult.created ? '商品已导入为新草稿' : '本地草稿已更新' }}。
          </template>
        </div>
        <ul v-if="warnings.length" class="warning-list">
          <li v-for="warning in warnings" :key="warning">{{ warning }}</li>
