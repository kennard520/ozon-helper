...
// 每草稿持久化的 loading：可写计算属性，读写都落到 store.pendingOps[草稿id][name]。
// 用法同 ref(xLoading.value = true/false; 模板 :loading="xLoading")，但切草稿(组件因 :key 重建)
// 再切回时，未完成的操作按钮仍能读回 loading 态、继续转圈；操作 finally 清除后自动停。
function useOp(name) {
  return computed({
    get: () => store.isOp(props.draft && props.draft.id, name),
    set: (v) => store.setOp(props.draft && props.draft.id, name, v),
  })
}
...
const aiGenerating = useOp('aiGenerate')
const publishingGroup = useOp('publishGroup')
...
// 变体组兄弟清单（每变体一张草稿，靠 variant_group 归一）：编辑器里展示「共N变体」+ 区别规格，可点切换/整组发布
const variantSiblings = ref({ group: '', variants: [], count: 0 })
async function loadVariantGroup() {
  if (!variantGroup.value) { variantSiblings.value = { group: '', variants: [], count: 0 }; return }
  try { variantSiblings.value = await api.variantGroup(props.draft.id) }
  catch { variantSiblings.value = { group: '', variants: [], count: 0 } }
}
...
const imgGenN = ref(3)
const imgGenLoading = useOp('imgGen')
...
// 每属性的 loading 也走 store(键 attr:<id> / attr:ai)，切草稿再切回仍能续转。读写沿用 attrLoading[id]=布尔
const attrLoading = new Proxy({}, {
  get: (_t, k) => (typeof k === 'symbol' ? undefined : store.isOp(props.draft && props.draft.id, 'attr:' + String(k))),
  set: (_t, k, v) => { if (typeof k !== 'symbol') store.setOp(props.draft && props.draft.id, 'attr:' + String(k), v); return true },
})
...
const understandLoading = useOp('understand')
const recommendLoading = useOp('recommend')
...
const autoImgLoading = useOp('autoImg')
const applyLoading = useOp('applyCands')
...
async function doApplyCandidates() {
  ...
  try {
    ...
    if (r && r.ok) {
      // 应用只改了 images(+image_types)，只更新图片文本即可——不跑整个 initFromDraft(重建全部属性,慢)
      if (r.draft) { store.upsertDraft(r.draft); imagesText.value = (Array.isArray(r.draft.images) ? r.draft.images : []).join('\n') }
      ElMessage.success(`已应用 ${r.added} 张到图集`)
    }
...
const ssLoading = useOp('slideshow')
...
const batchSel = ref([])
const batchLoading = useOp('batch')
