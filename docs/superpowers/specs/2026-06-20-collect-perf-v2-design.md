...
1. **原产国写死“中国”**：`auto_map_attributes` 在匹配属性时，识别原产国属性（采集属性名含 `Страна`，即俄文“国家/制造国”），把它的值**强制设为 `Китай`（中国）**，再走正常映射（缓存命中、快）。业务正确：跨境一律从中国发货。
...
- **属性**：原产国被强制成“中国”；并发 16 解析结果正确；全属性映射不漏。
